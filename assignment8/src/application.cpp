...
void Application::init() {
  ...
  // Create two ropes
  const int my_node_count = 16;
  ropeEuler = new Rope(Vector2D(0, 200), Vector2D(-400, 200), my_node_count, config.mass,
                       config.ks, {0});
  ropeVerlet = new Rope(Vector2D(0, 200), Vector2D(-400, 200), my_node_count, config.mass,
                        config.ks, {0});
}
